...
const { redisUrl, redisPort, redisDbIndex } = require('../../config/vars');

Promise.promisifyAll(redis);

const client = redis.createClient({ host: redisUrl, port: redisPort });

client.on('connect', () => {
  client.select(redisDbIndex);
  logger.info(`Redis client connected to database index ${redisDbIndex}`);
});
